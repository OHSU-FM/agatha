class RoleAggregate < ActiveRecord::Base
  has_paper_trail
  attr_accessible :virtual_survey_sid, :agg_fieldname, :agg_label,
    :agg_title_fieldname, :pk_fieldname, :pk_title_fieldname, :pk_label,
    :default_view, :managed_permissions
  ...
  def lime_survey
    return nil unless virtual_survey_sid
    @lime_survey ||= LimeExt::VirtualSurvey.new(virtual_survey_sid, rc_api)
  end
  ...
  def ready_for_use?
    agg_question.present? && pk_question.present?
  end
  ...
  def pk_question
    ...
    @pk_question = lime_survey.find_question :rc_resp_name, pk_fieldname
  end
  ...
  def pk_title_question
    ...
    @pk_title_question = lime_survey.find_question :rc_resp_name, pk_title_fieldname
  end
  ...
  def agg_question
    ...
    @agg_question = lime_survey.find_question :rc_resp_name, agg_fieldname
  end
  ...
  def agg_title_question
    ...
    @agg_title_question = lime_survey.find_question :rc_resp_name, agg_title_fieldname
  end
  ...
  def find_fieldname_str fieldname
    enum_column_names.find{|val| val == fieldname }
  end
  ...
  def enum_column_names
    status_cols = lime_survey.status_questions.map{|sq| sq.rc_resp_name }.to_set
    lime_survey.column_names.select{|title, col_name|
      !status_cols.include?(col_name)
    }
  end
