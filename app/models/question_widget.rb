class QuestionWidget < ActiveRecord::Base
  ...
  validates_inclusion_of :pk,
    ...
    :if => lambda { |obj| obj.pk.to_s == "_" ? false : obj.pk_changed? }
  ...
  def lime_question_enum
    return [] unless role_aggregate
    role_aggregate.lime_survey.virtual_questions.map{|q|[q.title, q.qid] unless q.hidden?}.compact
  end
  ...
  def content
    begin
      result = {
        ...
        :widget_title=>"#{fm.lime_survey.title}: #{stats.question.group.group_name}(#{stats.question.title})"
      }
  ...
  def fm
    ...
    @fm ||= LimeExt::FilterManager.new user, role_aggregate.lime_survey_sid,
      LimeExt::API.new(), :params=>{:agg=>agg, :pk=>pk}
  end
